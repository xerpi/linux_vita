...
// Based on drivers/clk/clk-bd718x7.c and drivers/clk/clk-apple-gate.c
...
static int vita_clk_gate_endisable(struct clk_hw *hw, int enable)
{
	...
	printk("Vita clock gate en/disable, reg: 0x%X, enable: %d\n", (u32)gate->reg, enable);
...
MODULE_DESCRIPTION("PlayStation Vita clk gate driver");
MODULE_LICENSE("GPL");
