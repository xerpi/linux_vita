...
static inline void vita_spi_read_fifo_wait(struct vita_spi *vspi, int n)
{
	while (vita_spi_read(vspi, 0xA * 4) < n)
		udelay(100);
}
...
static inline void vita_spi_set_mosi(struct vita_spi *vspi, int is_on)
{
        gpiod_set_value_cansleep(vspi->mosi, is_on);
}
...
static void vita_spi_transfer_recv(struct vita_spi *vspi, u8 *rx_buf, int len)
{
	...
	/*while (len >= 2) {
		vita_spi_read_fifo_wait(vspi, 2);
		val = vita_spi_read(vspi, 0 * 4);
		rx_buf[i] = val & 0xFF;
		rx_buf[i + 1] = (val >> 8) & 0xFF;
		i += 2;
	...
	if (len > 0) {
		vita_spi_read_fifo_wait(vspi, 1);
		rx_buf[i] = vita_spi_read(vspi, 0 * 4) & 0xFF;
	}*/
...
static irqreturn_t vita_spi_irq_handler(int irq, void *dev_id)
{
	...
	//u32 status = vita_spi_read(vspi, vita_INT_STA_REG);
...
static int vita_spi_probe(struct platform_device *pdev)
{
	...
	vspi->mosi = devm_gpiod_get_optional(&pdev->dev, "mosi", GPIOD_OUT_HIGH);
	if (IS_ERR(vspi->mosi))
		return PTR_ERR(vspi->mosi);
