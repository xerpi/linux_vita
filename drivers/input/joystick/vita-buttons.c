...
static void vita_syscon_buttons_poll(struct input_dev *input)
{
	...
	u8 data[SYSCON_RX_HEADER_SIZE + 10];
	u32 buttons;

	// Packet 0x101 is 10 bytes long
	// Packet 0x104 is 14 bytes long, includes arrows pressure
	pad->syscon->command_read(pad->syscon, 0x101, data, sizeof(data));
	...
	input_report_key(input, BTN_TL, buttons & CTRL_L);
	input_report_key(input, BTN_TR, buttons & CTRL_R);
