 ...
 *
 * Functions to access PLyaStation Vita's Syscon (Ernie)
 *
...
struct vita_syscon {
	...
	int (*command_read)(struct vita_syscon *syscon, u16 cmd, void *buffer, int length);
	int (*short_command_write)(struct vita_syscon *syscon, u16 cmd, u32 data, int length);
	int (*transfer)(struct vita_syscon *syscon, void *tx, int tx_size, void *rx, int max_rx_size);
	/* Syscon info */
	u32 baryon_version;
	u32 hardware_info;
